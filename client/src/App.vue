...
<template>
  <div class="app-root flex h-screen overflow-hidden font-sans transition-colors duration-300"
    :class="isDark ? 'bg-zinc-900' : 'bg-zinc-50'">

    <aside v-if="showShell" class="flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out border-r z-20"
      :class="[
        sidebarOpen ? 'w-56' : 'w-14',
        isDark ? 'bg-zinc-950 border-white/[.06]' : 'bg-white border-zinc-200'
      ]">
      <!-- Logo -->
      <div @click="router.push('/home')" class="flex items-center gap-2.5 px-3.5 border-b h-14 flex-shrink-0"
        ...
        <span v-if="sidebarOpen" class="text-sm font-semibold tracking-wide whitespace-nowrap overflow-hidden"
      ...
      <nav class="flex flex-col gap-1 p-2 flex-1 overflow-y-auto">
        <button v-for="item in navItems" :key="item.href" @click="navigate(item.href)"
          ...
          <span class="flex-shrink-0 w-4 h-4 flex items-center justify-center">
            ...
            <svg v-else-if="item.icon === 'doc'" class="w-4 h-4" viewBox="0 0 16 16" fill="none">
      ...
      <div class="flex flex-col gap-1 p-2 border-t flex-shrink-0"
        :class="isDark ? 'border-white/[.06]' : 'border-zinc-200'">
        <button @click="toggleTheme"
          class="flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition-all duration-150 w-full" :class="isDark
          ...
          <span class="flex-shrink-0 w-4 h-4 flex items-center justify-center">
            ...
            <svg v-else class="w-4 h-4" viewBox="0 0 16 16" fill="none">
              <path d="M13.5 10A6 6 0 016 2.5a6 6 0 100 11 6 6 0 007.5-3.5z" stroke="currentColor" stroke-width="1.3"
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      ...
      <header v-if="showShell" class="flex h-14 flex-shrink-0 items-center justify-between border-b px-6"
        :class="isDark ? 'bg-zinc-950 border-white/[.06]' : 'bg-white border-zinc-200'">
        <div class="flex items-center gap-3">
          <h1 class="text-sm font-semibold" :class="isDark ? 'text-white' : 'text-zinc-900'">
            {{ pageTitle }}
          </h1>
          <span class="rounded-full px-2 py-0.5 text-[.6rem] font-medium uppercase tracking-widest border" :class="isDark
    ...
    <Transition name="fade">
      <div v-if="drawerOpen" class="fixed inset-0 z-30 bg-black/40 backdrop-blur-[2px]" @click="closeDrawer" />
    </Transition>

    <Transition name="slide">
      <aside v-if="drawerOpen" class="fixed right-0 top-0 z-40 flex h-full w-full max-w-md flex-col shadow-2xl"
        :class="isDark ? 'bg-zinc-900' : 'bg-white'">
        <div class="flex h-14 flex-shrink-0 items-center justify-between border-b px-5"
          :class="isDark ? 'border-white/[.06]' : 'border-zinc-200'">
          <div class="flex items-center gap-2.5">
            <div class="h-1.5 w-1.5 rounded-full bg-blue-500" />
            <span class="text-sm font-semibold" :class="isDark ? 'text-white' : 'text-zinc-900'">
              {{ drawerItem?.label }}
            </span>
          </div>
          <button @click="closeDrawer" class="flex h-7 w-7 items-center justify-center rounded-lg transition-colors"
            :class="isDark
        ...
        <div class="flex flex-shrink-0 items-center justify-end gap-2 border-t px-5 py-3.5"
          :class="isDark ? 'border-white/[.06]' : 'border-zinc-200'">
          <button @click="closeDrawer" class="rounded-lg border px-4 py-2 text-xs font-medium transition-all" :class="isDark
            ? 'border-zinc-700 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200'
            : 'border-zinc-200 text-zinc-500 hover:border-zinc-300 hover:text-zinc-700'">
            Cancelar
          </button>
          <button
            class="rounded-lg bg-blue-500 px-4 py-2 text-xs font-semibold text-white transition-all hover:bg-blue-400 active:scale-[.98]">
            Salvar
          </button>
        </div>
      </aside>
